...
describe("language harnesses", () => {
  it.each(["cpp", "java", "python", "javascript"] as const)(
    "builds a %s sample harness",
    (language) => {
  ...
  it("supports LeetCode-style Solution wrappers", () => {
    const python = buildHarness(
      question,
      "python",
      "class Solution:\n    def add(self, a: int, b: int) -> int:\n        return a + b",
      "RUN",
    );
    const cpp = buildHarness(
      question,
      "cpp",
      "class Solution { public: int add(int a, int b) { return a + b; } };",
      "RUN",
    );
    const java = buildHarness(
      question,
      "java",
      "class Solution { public int add(int a, int b) { return a + b; } }",
      "RUN",
    ...
    expect(python.source).toContain("Solution().add");
    expect(cpp.source).toContain("Solution instance;");
    expect(java.source).toContain("outer.new Solution()");
  });
});
