...
export interface Harness { source: string; tests: TestCase[] }

function selectedTests(question: CodeQuestion, mode: "RUN" | "SUBMIT") {
  return question.tests.filter((test) => mode === "RUN" ? test.visibility === "SAMPLE" : test.visibility === "HIDDEN");
}

function cppType(value: unknown): string {
  if (Array.isArray(value)) return `vector<${value.length ? cppType(value[0]) : "int"}>`;
...
function cppLiteral(value: unknown): string {
  if (Array.isArray(value)) return `${cppType(value)}{${value.map(cppLiteral).join(",")}}`;
...
function javaType(value: unknown): string {
  if (Array.isArray(value)) return `${value.length ? javaType(value[0]) : "int"}[]`;
  if (typeof value === "string") return "String";
...
function javaLiteral(value: unknown): string {
  if (Array.isArray(value)) return `new ${javaType(value)}{${value.map(javaLiteral).join(",")}}`;
  if (typeof value === "string") return JSON.stringify(value);
...
function javascriptHarness(question: CodeQuestion, source: string, tests: TestCase[]) {
...
function pythonHarness(question: CodeQuestion, source: string, tests: TestCase[]) {
...
function cppHarness(question: CodeQuestion, source: string, tests: TestCase[]) {
  const cases = tests.map((test, testIndex) => {
    const args = Array.isArray(test.input) ? test.input : [test.input];
    const declarations = args.map((value, index) => `${cppType(value)} a${testIndex}_${index} = ${cppLiteral(value)};`).join("\n");
    const callArgs = args.map((_, index) => `a${testIndex}_${index}`).join(",");
...
function javaHarness(question: CodeQuestion, source: string, tests: TestCase[]) {
  const cases = tests.map((test, index) => {
    const args = (Array.isArray(test.input) ? test.input : [test.input]).map(javaLiteral).join(",");
...
export function buildHarness(question: CodeQuestion, language: Language, source: string, mode: "RUN" | "SUBMIT"): Harness {
  const tests = selectedTests(question, mode);
  const builders = { javascript: javascriptHarness, python: pythonHarness, cpp: cppHarness, java: javaHarness };
