# Teacher Integrity Review Rules
...
- Validate changes with `npm run lint --workspace teacher` and `npm run build --workspace teacher`, then exercise `/reviews` with a local teacher session.
